...
class GitAlchemistConfig(BaseModel):
    """
    Represents the configuration of an GitAlchemist task and is shared between its commands.
    It contains information on different paths that are needed for repository generation
    and git command execution.

    Fields:
    - task (str): the task name. This is equivalent to the directory containing the gitalchemist.yaml file in config_dir.
    - root_dir (Path): top-level directory.
    - config_dir (Path): directory containing task directories (field 'task') of name task.
    - working_dir (Path): directory where git repositories should be built.
      Should be a path that is relative to root_dir. Will be created as a subdirectory of root_dir.
    - bare_dir (Path): directory of a bare repository to clone from and push to,
      which is the first step in all GitAlchemist tasks. Is provided by gitalchemist.yaml
      configuration file and set by CMDInitBareRepo.
    - repo_dir (Path): directory of the git repository that is active in a task.
      Is provided by gitalchemist.yaml configuration file and set by CMDInitBareRepo.
    - current_repo (str): name of the currently active git repository.
      Is provided by gitalchemist.yaml and set by CMDInitBareRepo.
